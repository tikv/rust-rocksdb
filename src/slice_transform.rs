...
pub trait SliceTransform {
    ...
    fn transform<'a>(&mut self, key: &'a [i8]) -> &'a [i8];
...
extern "C" fn in_domain(transform: *mut c_void, key: *const i8, key_len: size_t) -> u8 {
    unsafe {
        let transform = &mut *(transform as *mut SliceTransformProxy);
        let key = slice::from_raw_parts(key, key_len);
...
extern "C" fn in_range(transform: *mut c_void, key: *const i8, key_len: size_t) -> u8 {
    unsafe {
        let transform = &mut *(transform as *mut SliceTransformProxy);
        let key = slice::from_raw_parts(key, key_len);
